...
{
    /// <summary>
    /// 场景复制工具：将源场景目录完整复制到目标目录，
    /// 并将所有 Asset 引用从源路径批量重定向到目标路径。
    /// </summary>
    public partial class CopyScene
    {
        /// <summary>
        /// 文件过滤后缀常量：复制时跳过 SVN、Meta 等辅助文件，
        /// 并识别 Asset、材质、Prefab、场景、动画控制器等资产格式。
        /// </summary>
        ...
        /// <summary>
        /// 将源场景目录复制到目标目录下的同名子目录，并重写所有 Asset 引用。
        /// </summary>
        /// <param name="srcDir">源场景根目录的绝对路径。</param>
        /// <param name="dstDir">目标父目录的绝对路径。</param>
        /// <param name="sceneName">目标场景名称（用作子目录名）。</param>
